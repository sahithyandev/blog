...
Hence, I decided to design the whole UI first (or at least try). I opened up [Figma](https://figma.com) and gave it a go. Here's what I designed.

<img alt="XCalc Old UI design" src="/post__images/building-xcalc/xcalc-old-design.jpg" />

I know it's crappy. But I went with it anyways. Because, if I spend too much time on my UI design, trying to make it *perfect*, I will never finish it.  

After about 3 months of break, I came back to the project, and decided to give it a redesign. To be honest, it turned out to be pretty great.
...
By default, the browser uses a single thread (main thread) to run every processes on a website. This is where the layout and paint are done. This is where the user events get processed. And also, all of the javascript runs here. Which means, long-running JavaScript can block the thread, leading to an unresponsive page (== bad user experience)

This is where web workers come into play. Web workers are a simple means for web content to run scripts on background threads. This will make sure the main thread is free from getting stuck. [Learn more about web workers](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Using_web_workers).

When I tested it on a laptop and my phone, XCalc's main thread didn't get blocked. There were no performance issues. But, it might happen on the low-end phones. To eliminate that possibility, I planned to move the evaluation of XCalc to a web worker.
...
C, C++, Rust already offered a way to compile to WASM, but I didn't want to learn a whole new language just to try out a new technology. Hence, I continued searching. Luckily, I came across [AssemblyScript](https://www.assemblyscript.org). AssemblyScript had a typescript-like syntax, and it worked a lot like TypeScript. It seemed like the best choice for me, so I went with it.
...
**Try to design the whole UI before starting developing the frontend**. It may seem useless and a waste of time. But, believe me, it’s not. Anyways, don’t try to make it *perfect*. And remember, I didn’t say you “must” design the whole UI. Just try. Just make a crappy quick draft.
